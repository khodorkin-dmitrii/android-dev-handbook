...
Effects - временные команды: показать snackbar, выполнить навигацию, прокрутить список или открыть системный picker. Отдельный `SharedFlow` или [Channel](channels.md) подходит, если результат некритичен, а поведение без активного коллектора определено заранее.
