...
Certificate pinning сужает набор серверных идентификаторов, которым приложение доверяет помимо стандартной проверки платформы. Он может снизить отдельные риски, связанные с certificate authorities, но усложняет развертывание, восстановление, тестирование и rotation. Pinning - это дополнительное ограничение после обычной TLS validation, а не обязательная настройка каждого Android-приложения.

## Какую задачу решает pinning

Стандартный TLS принимает корректную certificate chain, ведущую к любому trust anchor, разрешенному приложением. Pinning добавляет дополнительное требование для конкретного host: хотя бы один сертификат в уже проверенной peer chain должен содержать ожидаемый public key. Это может снизить риск ошибочно или злонамеренно выпущенного сертификата или неожиданно доверенного CA.
...
OkHttp `CertificatePinner` использует SPKI hashes. Это упрощает обновление сертификата, если public key остается прежним, однако бесконечное повторное использование ключа увеличивает ущерб от его компрометации. Команде всё равно необходимы заранее продуманная rotation и backup pins. Browser HPKP - устаревший web-механизм; конфигурация OkHttp на уровне приложения имеет другие ограничения развертывания и восстановления, хотя формат хеша связан с HPKP.
...
Надежная политика обычно принимает как минимум primary и backup pin. Резервный pin должен соответствовать отдельному ключу, который команда контролирует независимо от основного, безопасно хранит и при необходимости может использовать для получения сертификата и развертывания на сервере. Hash без доступа к соответствующему private key и issuance path не является рабочим backup.

Закрепление каждого сертификата в текущей цепочке не гарантирует надежность. Intermediates могут быть общими, меняться по решению CA или находиться вне контроля приложения. Нужно явно определить, какими ключами владеет организация и какие изменения инфраструктуры может пережить версия приложения.
...
3. Дождаться, пока новую версию установит достаточная доля пользователей, с учетом поддерживаемых старых версий.
4. Развернуть новый server certificate и key.
5. Проверить production-трафик и совместимость со старыми версиями приложения.
6. Удалить устаревшие pins только в следующем release, когда это позволяет политика поддержки.

Server configuration меняется за минуты, но выпуск мобильного приложения требует review, а часть пользователей никогда не обновится. Поэтому самая старая поддерживаемая версия приложения тоже влияет на операции с сертификатами.

## Истечение срока действия и стратегия восстановления

До включения pinning нужно подготовиться к истекшему сертификату, скомпрометированному ключу, ошибочному pin, экстренной миграции и постоянно устаревшим версиям приложения. Ошибочный pin может привести к полной недоступности API, при которой приложение не достигнет даже сервиса доставки новой конфигурации.

Полезные меры включают независимо используемые backup pins, overlapping validity, поэтапное развертывание изменений в приложении и на сервере, monitoring и определенную minimum supported version. Альтернативный endpoint может использоваться для recovery только в том случае, если его identity и поведение заранее спроектированы и защищены. Загрузка произвольного replacement pin через уже недоверенное соединение уничтожает смысл pinning.

Для remote configuration возникает та же bootstrap-проблема. Она может выбирать среди pins или policies, уже аутентифицированных установленным приложением, но не может безопасно создать новый trust anchor после отказа всех pinned paths без отдельного authenticated recovery mechanism.

## Операционные риски

- Несовпадение pin может заблокировать весь API-трафик для затронутых версий.
...
Не закрепляйте сторонние endpoints, если provider явно не поддерживает pinning и не гарантирует стабильный процесс rotation сертификатов. Сторона, меняющая сертификат, должна участвовать в lifecycle pins.

## Влияние на QA и инструменты перехвата трафика

Interception proxy предъявляет свою certificate chain, поэтому pinned production host обычно ее отклоняет, даже когда debug app доверяет proxy CA. Глобальное отключение pinning или пользовательский переключатель приводят к тому, что поведение production-сборки перестает быть предсказуемым.
...
| Старый и новый pins во время rotation | Оба варианта развертывания работают | Staging |
...
Unit tests могут проверять выбор конфигурации и host-pattern logic. Для реального TLS, chain building, proxy и rotation нужны integration tests или staging infrastructure.
